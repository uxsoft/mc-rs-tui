...
use crate::vfs_archive::{decompress_capped, safe_archive_key};
...
impl TarVfs {
    ...
    pub fn open_compressed(
        path: &Path,
        scheme: &'static str,
        c: crate::vfs_archive::Compression,
    ) -> Result<Self> {
        let f = std::fs::File::open(path).map_err(Error::Io)?;
        let buf = match c {
            crate::vfs_archive::Compression::Gz => {
                decompress_capped(flate2::read::GzDecoder::new(f))?
            }
            crate::vfs_archive::Compression::Bz2 => {
                decompress_capped(bzip2::read::BzDecoder::new(f))?
            }
            crate::vfs_archive::Compression::Xz => decompress_capped(xz2::read::XzDecoder::new(f))?,
            crate::vfs_archive::Compression::Zst => {
...
mod tests {
    ...
    async fn open_and_list_uncompressed_tar() {
        ...
        let root = VPath::new([crate::core::path::Layer {
            scheme: "tar".into(),
            location: String::new(),
            sub: "/".into(),
        }]);
        ...
        let inside = VPath::new([crate::core::path::Layer {
            scheme: "tar".into(),
            location: String::new(),
            sub: "/dir".into(),
        }]);
        ...
        let file_path = VPath::new([crate::core::path::Layer {
            scheme: "tar".into(),
            location: String::new(),
