...
pub fn decompress_capped<R: Read>(r: R) -> crate::core::Result<Vec<u8>> {
    decompress_capped_with(r, MAX_DECOMPRESSED)
}

fn decompress_capped_with<R: Read>(r: R, limit: u64) -> crate::core::Result<Vec<u8>> {
...
/// `scheme`. The scheme becomes the layer name in [`crate::core::VPath`].
pub fn mount_local(
    path: &Path,
    kind: ArchiveKind,
    scheme: &'static str,
) -> crate::core::Result<Arc<dyn Vfs>> {
    match kind {
