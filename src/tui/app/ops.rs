...
use crate::tui::dialog::{ConfirmDialog, CopyMoveSettingsDialog};
use crate::tui::panel::{ListingMode, PanelState};
...
impl App {
    ...
    pub(super) fn try_mount_archive(&mut self, target: &VPath) -> Option<VPath> {
        ...
        let kind = crate::vfs_archive::ArchiveKind::detect(&local_path)?;
        let scheme: &'static str = match kind {
            crate::vfs_archive::ArchiveKind::Tar
            | crate::vfs_archive::ArchiveKind::TarGz
            | crate::vfs_archive::ArchiveKind::TarBz2
            | crate::vfs_archive::ArchiveKind::TarXz
            | crate::vfs_archive::ArchiveKind::TarZst => "tar",
            crate::vfs_archive::ArchiveKind::Zip => "zip",
            crate::vfs_archive::ArchiveKind::Cpio => "cpio",
            crate::vfs_archive::ArchiveKind::SevenZ => "7z",
            #[cfg(feature = "rar")]
            crate::vfs_archive::ArchiveKind::Rar => "rar",
        };
        let mount_id = next_mount_id();
        let location = format!("mount-{mount_id}");
        match crate::vfs_archive::mount_local(&local_path, kind, scheme) {
            Ok(vfs) => {
                self.registry.register_mount(scheme, location.clone(), vfs);
                let mut p = target.clone();
                p.push_layer(crate::core::path::Layer {
    ...
    pub(super) fn open_diff(&mut self) -> Disposition {
        ...
        match crate::tui::diff_widget::DiffWidget::open(&l_local, &r_local) {
            Ok(d) => {
    ...
    pub(super) fn macro_ctx(&self) -> crate::core::MacroCtx {
        let active = self.active_ref();
        let other = if self.active_left {
            &self.right
        } else {
    ...
    pub(super) fn open_copy_move_with(
        ...
        self.modal = Modal::CopyMove {
            dlg: CopyMoveSettingsDialog::new(
                ...
                &prompt,
                prefilled,
                crate::jobs::CopyOptions::default(),
            ),
            sources,
            src_cwd,
            kind,
...
pub(super) fn apply_panel_snapshot(p: &mut PanelState, s: &crate::config::PanelStateSnapshot) {
    p.sort_by = s.sort_by;
    p.reverse = s.reverse;
...
pub(super) fn panel_snapshot(p: &PanelState) -> crate::config::PanelStateSnapshot {
    crate::config::PanelStateSnapshot {
        sort_by: p.sort_by,
        reverse: p.reverse,
